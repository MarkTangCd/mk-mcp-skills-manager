...
use crate::domain::{AgentKind, PromptTemplate};
...
impl LibraryService {
    ...
    pub fn delete(&self, kind: LibraryKind, slug: &str) -> LibraryResult<()> {
        ...
        self.guard.ensure_writable(entry_dir.join(".guard-check"))?;
    ...
    pub fn skills_import(
        ...
        let slug = match slug {
            ...
            None => source
                ...
                .and_then(|n| n.to_str())
                .map(|s| s.to_string())
                .ok_or_else(|| LibraryError::InvalidSlug("derived slug is empty".to_string()))?,
        };
    ...
    pub fn prompts_list(
        ...
        search: Option<&str>,
        tags: Option<&[String]>,
    ) -> LibraryResult<Vec<PromptTemplate>> {
        let dir = self.kind_dir(LibraryKind::Prompts);
        ...
        for entry in std::fs::read_dir(&dir)? {
            let entry = entry?;
            let path = entry.path().join("prompt.json");
            if path.exists() {
                prompts.push(self.read_prompt(&path)?);
            }
        }
        ...
        if let Some(query) = search {
            ...
            prompts.retain(|prompt| {
                prompt.slug.to_lowercase().contains(&q)
                    || prompt.title.to_lowercase().contains(&q)
                    || prompt
                        .category
                        .as_deref()
                        .unwrap_or("")
                        .to_lowercase()
                        .contains(&q)
                    || prompt.body.to_lowercase().contains(&q)
                    || prompt
                        .tags
                        .iter()
        ...
        if let Some(required_tags) = tags {
            if !required_tags.is_empty() {
                prompts.retain(|prompt| required_tags.iter().all(|tag| prompt.tags.contains(tag)));
            }
        }
    ...
    pub fn prompts_create(
        ...
        favorite: bool,
        body: &str,
    ) -> LibraryResult<PromptTemplate> {
        if !is_valid_slug(slug) {
            return Err(LibraryError::InvalidSlug(slug.to_string()));
        }

        let entry_dir = self.entry_dir(LibraryKind::Prompts, slug);
        if entry_dir.exists() {
            return Err(LibraryError::DuplicateSlug(slug.to_string()));
        }
        self.guard.ensure_writable(&entry_dir)?;
        ...
        let prompt = PromptTemplate {
            ...
            body: body.to_string(),
            variables: extract_prompt_variables(body),
            tags,
            favorite,
        };
        self.write_json_atomic(&entry_dir.join("prompt.json"), &prompt)?;
        Ok(prompt)
    }
    ...
    pub fn prompts_update(
        ...
        if !entry_dir.exists() {
            return Err(LibraryError::NotFound(
                LibraryKind::Prompts.as_str().to_string(),
                slug.to_string(),
            ));
        }
    ...
    pub fn prompts_get(&self, slug: &str) -> LibraryResult<PromptTemplate> {
        let path = self
            .entry_dir(LibraryKind::Prompts, slug)
            .join("prompt.json");
        if !path.exists() {
            return Err(LibraryError::NotFound(
                LibraryKind::Prompts.as_str().to_string(),
    ...
    pub fn prompts_delete(&self, slug: &str) -> LibraryResult<()> {
        self.delete(LibraryKind::Prompts, slug)
    }
    ...
    ) -> LibraryResult<PromptRenderResult> {
        let prompt = self.prompts_get(slug)?;
        Ok(render_prompt(&prompt, values))
    }
    ...
    fn read_prompt(&self, path: &Path) -> LibraryResult<PromptTemplate> {
        let content = std::fs::read_to_string(path)?;
        let mut prompt: PromptTemplate = serde_json::from_str(&content)?;
        prompt.variables = extract_prompt_variables(&prompt.body);
        Ok(prompt)
    }

    fn write_json_atomic(&self, path: &Path, value: &impl Serialize) -> LibraryResult<()> {
        self.guard.ensure_writable(path)?;

        let parent = path
            .parent()
            .ok_or_else(|| LibraryError::PathNotAllowed(path.to_string_lossy().to_string()))?;
...
fn extract_prompt_variables(body: &str) -> Vec<String> {
    ...
    while let Some(start) = rest.find("{{") {
        ...
        if is_valid_prompt_variable(name) && !variables.iter().any(|existing| existing == name) {
...
fn is_valid_prompt_variable(name: &str) -> bool {
...
fn render_prompt(prompt: &PromptTemplate, values: &HashMap<String, String>) -> PromptRenderResult {
    let mut content = prompt.body.clone();
    let mut missing_variables = Vec::new();

    for variable in &prompt.variables {
        let token = format!("{{{{{variable}}}}}");
        match values.get(variable).filter(|value| !value.is_empty()) {
            Some(value) => content = content.replace(&token, value),
            None => missing_variables.push(variable.clone()),
        }
    }
...
mod tests {
    ...
    fn list_entries() {
        ...
        svc.create(LibraryKind::Skills, "skill-a", metadata1)
            .unwrap();
        svc.create(LibraryKind::Skills, "skill-b", metadata2)
            .unwrap();
    ...
    fn update_entry() {
        ...
        assert_eq!(detail.metadata.description, Some("New desc".to_string()));
    }
    ...
    fn write_uses_atomic_rename() {
        ...
        svc.create(LibraryKind::Skills, "atomic", metadata).unwrap();
    ...
    fn skills_create_with_entry_file() {
        let (dir, svc) = svc();
        let entry = svc
            .skills_create("with-entry", "With Entry", None, vec![], Some("skill.md"))
            .unwrap();
        ...
        assert!(
            ...
            "entry file placeholder should exist"
        );
    ...
    fn skills_import_copies_directory() {
        ...
        assert_eq!(
            std::fs::read_to_string(&imported_readme).unwrap(),
            "# Hello"
        );
    }
    ...
    fn skills_import_rejects_invalid_slug() {
        ...
        let err = svc
            .skills_import(source.to_str().unwrap(), None)
            .unwrap_err();
        assert!(matches!(err, LibraryError::InvalidSlug(_)));
    }
    ...
    fn sub_agents_list_filters_by_search() {
        ...
        let by_tag = svc
            .sub_agents_list(None, Some(&["tag-a".to_string()]))
            .unwrap();
        assert_eq!(by_tag.len(), 1);
    ...
    #[test]
    fn prompt_create_extracts_unique_variables_in_order() {
        let (_dir, svc) = svc();
        ...
        let prompt = svc
            .prompts_create(
                ...
                true,
                "Review {{diff}} for {{focus}}. Then summarize {{diff}}.",
            )
            .unwrap();

        assert_eq!(prompt.variables, vec!["diff", "focus"]);
        assert_eq!(prompt.category.as_deref(), Some("review"));
        assert!(prompt.favorite);
    }

    #[test]
    fn prompt_render_reports_missing_variables_and_renders_values() {
        let (_dir, svc) = svc();
        svc.prompts_create(
            "debug-error",
            "Debug Error",
            ...
            vec![],
            false,
            "Debug {{errorMessage}} in {{filePath}}.",
        )
        .unwrap();
        ...
        let missing = svc
            .prompts_render(
                "debug-error",
                &HashMap::from([("errorMessage".to_string(), "panic".to_string())]),
            )
            .unwrap();
        assert_eq!(missing.missing_variables, vec!["filePath"]);

        let rendered = svc
            .prompts_render(
                "debug-error",
                &HashMap::from([
                    ("errorMessage".to_string(), "panic".to_string()),
                    ("filePath".to_string(), "src/main.rs".to_string()),
                ]),
            )
            .unwrap();

        assert_eq!(rendered.missing_variables, Vec::<String>::new());
        assert_eq!(rendered.content, "Debug panic in src/main.rs.");
    }

    #[test]
    fn prompt_search_matches_title_body_tags_and_category() {
        let (_dir, svc) = svc();
        svc.prompts_create(
            "ship-note",
            "Ship Note",
            ...
            false,
            "Write notes for {{version}}.",
        )
        .unwrap();

        assert_eq!(svc.prompts_list(Some("ship"), None).unwrap().len(), 1);
        assert_eq!(svc.prompts_list(Some("version"), None).unwrap().len(), 1);
        assert_eq!(svc.prompts_list(Some("release"), None).unwrap().len(), 1);
        assert_eq!(
            svc.prompts_list(None, Some(&["changelog".to_string()]))
                .unwrap()
                .len(),
            1
        );
